...
@section('title', 'Edit Geodiversitas')
...
<div style="background:white; border-radius:20px; box-shadow:0 8px 30px rgba(0,0,0,0.08); overflow:hidden;">
    <div style="padding:18px 24px; background:linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border-bottom:1px solid #e2e8f0;">
        <h5 style="margin:0; font-size:1.1rem; font-weight:700; color:#003366; display:flex; align-items:center; gap:8px;">
            <i class="fas fa-edit" style="color:#c6a43b;"></i> Edit Geodiversitas
        </h5>
    </div>
    <div style="padding:24px;">
        ...
        <form action="{{ route('admin.geodiversitas.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom:20px;">
                ...
                <select name="tipe_geologi" class="form-control" style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px;" required>
                    ...
                    <option value="batuan" {{ old('tipe_geologi', $data->tipe_geologi) == 'batuan' ? 'selected' : '' }}>Batuan</option>
                    <option value="mineral" {{ old('tipe_geologi', $data->tipe_geologi) == 'mineral' ? 'selected' : '' }}>Mineral</option>
                    <option value="fosil" {{ old('tipe_geologi', $data->tipe_geologi) == 'fosil' ? 'selected' : '' }}>Fosil</option>
                    <option value="formasi" {{ old('tipe_geologi', $data->tipe_geologi) == 'formasi' ? 'selected' : '' }}>Formasi</option>
                </select>
            </div>
...
<div style="display:flex; flex-wrap:wrap; gap:20px;">
                ...
                <div style="flex:1; min-width:200px;">
                    <div style="margin-bottom:20px;">
                        ...
                        <input type="text" name="usia_geologi" class="form-control" value="{{ old('usia_geologi', $data->usia_geologi) }}" placeholder="Contoh: 74.000 tahun" style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px;">
                    </div>
                </div>
            </div>
