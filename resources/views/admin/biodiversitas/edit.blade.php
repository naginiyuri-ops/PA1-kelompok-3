...
@section('title', 'Edit Biodiversitas')
...
<div style="background:white; border-radius:20px; box-shadow:0 8px 30px rgba(0,0,0,0.08); overflow:hidden;">
    <div style="padding:18px 24px; background:linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border-bottom:1px solid #e2e8f0;">
        <h5 style="margin:0; font-size:1.1rem; font-weight:700; color:#003366; display:flex; align-items:center; gap:8px;">
            <i class="fas fa-edit" style="color:#c6a43b;"></i> Edit Biodiversitas
        </h5>
    </div>
    <div style="padding:24px;">
        ...
        <form action="{{ route('admin.biodiversitas.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom:20px;">
                ...
                <input type="text" name="nama" id="bio_edit_nama" class="form-control"
                    value="{{ old('nama', $data->nama) }}"
                    style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px;" required>
            </div>
            ...
            <div style="margin-bottom:20px;">
                ...
                <select name="kategori" class="form-control" style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px;" required>
                    ...
                    <option value="flora"     {{ old('kategori', $data->kategori) == 'flora'     ? 'selected' : '' }}>Flora (Tumbuhan)</option>
                    <option value="fauna"     {{ old('kategori', $data->kategori) == 'fauna'     ? 'selected' : '' }}>Fauna (Hewan)</option>
                    <option value="ekosistem" {{ old('kategori', $data->kategori) == 'ekosistem' ? 'selected' : '' }}>Ekosistem</option>
                </select>
            </div>
            ...
            <div style="margin-bottom:20px;">
                ...
                <textarea name="deskripsi" id="bio_edit_deskripsi" class="form-control" rows="6"
                    style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px; resize:vertical;" required>{{ old('deskripsi', $data->deskripsi) }}</textarea>
            </div>
            ...
            @include('admin.partials.translation-fields', [
                ...
                'rowsDesc'       => 6,
                'sourceJudulId'  => 'bio_edit_nama',
                'sourceKontenId' => 'bio_edit_deskripsi',
            ])
            ...
            <div style="display:flex; flex-wrap:wrap; gap:20px;">
                ...
                <div style="flex:1; min-width:200px;">
                    <div style="margin-bottom:20px;">
                        <label style="display:block; margin-bottom:8px; font-weight:600; font-size:0.85rem; color:#1e293b;">Status Keberadaan</label>
                        <input type="text" name="status_keberadaan" class="form-control"
                            value="{{ old('status_keberadaan', $data->status_keberadaan) }}"
                            placeholder="Endemik, Dilindungi, Langka"
                            style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px;">
                    </div>
                </div>
            </div>
            ...
            <div style="margin-bottom:20px;">
                ...
                <input type="file" name="gambar" class="form-control" accept="image/*" id="inputGambar"
                    style="width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px;">
                <small style="font-size:0.7rem; color:#94a3b8; margin-top:5px; display:block;">Format: JPG, PNG, WEBP. Max: 2MB</small>
                @if($data->gambar && file_exists(public_path($data->gambar)))
                    <img id="previewImage" src="{{ asset($data->gambar) }}" style="max-width:150px; margin-top:10px; border-radius:10px;">
                @else
            ...
            <div style="display:flex; gap:12px; margin-top:24px;">
                <button type="submit"
                    style="background:linear-gradient(135deg, #003366 0%, #1a4a7a 100%); color:white; padding:10px 24px; border-radius:10px; font-weight:600; border:none; cursor:pointer;">
                    Simpan Perubahan
                </button>
                <a href="{{ route('admin.biodiversitas.index') }}"
                    style="background:#f1f5f9; color:#475569; padding:10px 24px; border-radius:10px; font-weight:600; border:none; cursor:pointer; text-decoration:none; display:inline-block;">
                    Batal
                </a>
            </div>
