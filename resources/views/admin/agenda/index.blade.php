...
<div class="card-table">
    ...
    <div class="stats-bar">
        <span class="stats-badge">
            <i class="fas fa-database"></i> Total: {{ $agenda->total() }}
        </span>
        <span class="stats-badge">
            <i class="fas fa-eye"></i> Halaman {{ $agenda->currentPage() }} / {{ $agenda->lastPage() }}
        </span>
    </div>
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td data-label="No">{{ $key + $agenda->firstItem() }}</td>
                        ...
                        @php
                            ...
                            if ($item->gambar) {
                                if (file_exists(public_path($item->gambar))) {
                                    ...
                                    $imgUrl = asset('image/agenda/' . $item->gambar);
                                }
                            }
                        @endphp
                    ...
                    <td data-label="Aksi">
                        <div class="btn-group">
                            <a href="{{ route('admin.agenda.edit', $item->id) }}" class="btn-edit">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <form action="{{ route('admin.agenda.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus agenda ini?')" style="display: inline;">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="7" class="empty-state">
                        <i class="fas fa-calendar-alt" style="font-size: 2rem; opacity: 0.5; display: block; margin-bottom: 10px;"></i>
                        📭 Belum ada data agenda
                        <p style="margin-top: 10px; font-size: 0.8rem;">Klik tombol "Tambah Agenda" untuk mulai menambahkan</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
