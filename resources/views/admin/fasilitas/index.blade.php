...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
    <h4 style="margin: 0;">🛠️ Manajemen Fasilitas Meat</h4>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="action-buttons">
                        ...
                        <form action="{{ route('admin.fasilitas.destroy', $item->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn-delete" onclick="return confirm('Yakin hapus fasilitas {{ $item->nama }}?')">Hapus</button>
                        </form>
...
<script>
    function toggleStatus(id, currentStatus) {
        ...
        if (confirm(`Yakin ingin ${action} fasilitas ini?`)) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `{{ url('admin/fasilitas/toggle-status') }}/${id}`;
            form.innerHTML = `@csrf`;
