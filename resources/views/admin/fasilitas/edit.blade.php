        ...
        <form action="{{ route('admin.fasilitas.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="tampil_as" id="tampil_as" class="form-control" required>
                    <option value="akomodasi" {{ old('tampil_as', $data->jenis) == 'akomodasi' ? 'selected' : ($data->jenis != 'kuliner' ? 'selected' : '') }}>Akomodasi</option>
                    <option value="kuliner" {{ old('tampil_as', $data->jenis) == 'kuliner' ? 'selected' : ($data->jenis == 'kuliner' ? 'selected' : '') }}>Kuliner / Restoran</option>
                </select>
            </div>
