...
<style>
    ...
    .agenda-card:hover .card-image-wrapper img {
    ...
    @media (max-width: 1024px) {
        .agenda-grid { grid-template-columns: repeat(2, 1fr); gap: 25px; }
        .hero-agenda h1 { font-size: 2.6rem; }
    }
    
    @media (max-width: 768px) {
        .hero-agenda h1 { font-size: 2rem; }
...
<section class="agenda-section">
    <div class="container">
        <div class="agenda-grid">
            ...
            <div class="agenda-card" data-aos="fade-up" data-aos-delay="{{ ($loop->index % 3) * 100 }}">
                @php
                    ...
                    if (!empty($item->gambar)) {
                        if (str_starts_with($item->gambar, 'data:image')) {
                            ...
                            $imgSrc = $item->gambar;
                        } elseif (str_starts_with($item->gambar, 'image/agenda/')) {
                            $imgSrc = asset($item->gambar);
                        } elseif (file_exists(public_path('image/agenda/' . $item->gambar))) {
                            $imgSrc = asset('image/agenda/' . $item->gambar);
                        } else {
                            $imgSrc = asset($item->gambar);
                        }
                    }
                ...
                <div class="card-image-wrapper" onclick="openAgenda({{ $item->id }})">
                ...
                <div class="card-content">
                    <h3 class="card-title" onclick="openAgenda({{ $item->id }})">{{ $item->judul_trans }}</h3>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="javascript:void(0)" class="read-more" onclick="openAgenda({{ $item->id }})">
...
<div id="readerModal" class="reader-modal">
    ...
    <div class="reader-nav">
        ...
        <button class="btn-close" onclick="closeAgenda()">
    ...
    <div class="reader-content">
        ...
        <div class="reader-footer">
            <button class="btn-back-reader" onclick="closeAgenda()">
                <i class="fas fa-arrow-left"></i> {{ __('app.common.back') }}
            </button>
            <button class="btn-share-reader" onclick="bagikanAgenda()">
...
<script>
    const agendaData = @json($agenda->items());
    
    async function openAgenda(id) {
        const item = agendaData.find(x => x.id === id);
        if (!item) return;
        ...
        if (item.gambar) {
            if (item.gambar.startsWith('data:image') || item.gambar.startsWith('http')) {
                imgSrc = item.gambar;
            } else if (item.gambar.startsWith('image/agenda/')) {
                imgSrc = '{{ asset("") }}' + item.gambar;
            } else if (item.gambar) {
                imgSrc = '{{ asset("image/agenda/") }}' + item.gambar;
            } else {
        ...
        try {
            const response = await fetch('/api/agenda/' + id + '/view', {
                method: 'POST',
                headers: {
